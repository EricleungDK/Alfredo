...
- Git snapshots retain a self-contained baseline bundle, the exact baseline commit and metadata, staged and unstaged binary patches, exact porcelain-v2 state digest, untracked non-ignored regular files with permission modes, untracked symlinks with their link targets, and all registered session artifacts. Unregistered directories are not retirement-eligible.
