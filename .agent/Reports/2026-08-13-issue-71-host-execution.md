...
- Pre-effect authorization, boundary-validation, sandbox-availability, command-not-found, and provider-start failures are typed `start-failed` and never treated as uncertain effects. A process is bound immediately after `Popen` and before capture setup; the child PID and start identity are atomically added to the executing journal record, so later setup failures remain uncertain rather than being mislabeled as a clean start failure. Process-group cleanup never signals a numeric group after its leader has been reaped and relies on the token-bound descendant set instead. Non-POSIX host effects fail closed. Journal writes fail closed on missing schema/receipt identity/raw-field redaction and fsync the containing directory after atomic replacement so the claim/receipt cut is durable across a crash.
