...
- Pre-effect authorization, boundary-validation, and provider-start failures are typed `start-failed` and never treated as uncertain effects. A process is bound immediately after `Popen` and before capture setup, so later setup failures remain uncertain rather than being mislabeled as a clean start failure. Journal writes fail closed on missing schema/receipt identity/raw-field redaction and fsync the containing directory after atomic replacement so the claim/receipt cut is durable across a crash.
