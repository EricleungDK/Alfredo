...
- When that exact stopped-runner proof includes an owner lease, recovery validates and removes only the matching Mission/session/operation/token lease before requeue. A missing or mismatched lease fails the intent closed, so a stale crashed-owner lease cannot strand the recovered session in `queued`.
