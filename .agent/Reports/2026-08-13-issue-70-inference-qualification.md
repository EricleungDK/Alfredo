...
- `QualificationReportStore` atomically persists bounded report metadata below `runtime_root/inference/qualification/`, serializes promotion state under a cross-process lock, pins the exact Profile/runtime/binary/configuration digests in both the report and every observation, requires the canonical fixture family and definition digests/repetitions, an existing comparable stored baseline, and a service-issued rollback-test receipt bound to an existing restored report, the candidate/runtime identity, and the previous-restored report identity. Reload revalidates the stored rollback target, report-root shape, derived outcome flags, finite accepted/repaired timings, reviewed-latency eligibility, and sequential prefix digest semantics. Atomic report/state writes translate replace failures to typed `PromotionError` values and clean temporary files. The cooperative rollback callback receives the same cancellation/deadline control seam; missing or mismatched stored restore evidence remains non-promotable, and promotion binds the receipt target to the active state or the qualified baseline retained for initial rollback. Inspection verifies that an active/previous pair is continuous with the active report's rollback receipt, rejects a promotion-ready active report with no retained target, and reconstructs every history request digest from its referenced report and revision before mutation. Promotion also compares canonical Profile configuration including the exact model digest and p50/p95 reviewed latency against the baseline, rejects withdrawn or non-inferior candidates, supports correlation/revision-guarded exact promotion and rollback replay, and retains an explicit rollback path to the previous active record. Report files, context cache bytes, and promotion history are each capped; incomplete or semantically invalid history cannot be mutated and exhausted histories fail closed so old correlation ids cannot be silently evicted. Deeply malformed bounded JSON is converted to typed persistence errors rather than leaking recursion failures.
...
| Safe promotion and rollback | `QualificationReportStore` tests cover exact runtime pinning and runner echo, control-error reliability, observation identity/prefix/timing invariants, withdrawn-runtime rejection, non-inferior quality/reliability/latency and model-drift baseline rejection, canonical fixture-definition binding, qualified-profile/fixture/repetition gates, rollback receipt and existing-restore baseline, initial-baseline rollback, rollback-state continuity, idempotent promotion, malformed report roots, typed atomic-write failure, persistence reload, semantic history/digest validation, history-capacity fail-closed behavior, and rollback. |

## Verification

- `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tests -p 'test_inference_qualification*.py' -q` — 39 passed (36 qualification/store tests plus 3 CLI/transport tests).
