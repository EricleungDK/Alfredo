...
| Safe promotion and rollback | `QualificationReportStore` tests cover exact runtime pinning and runner echo, observation identity/prefix/timing invariants, withdrawn-runtime rejection, non-inferior quality/reliability/latency and model-drift baseline rejection, qualified-profile/fixture/repetition gates, rollback receipt and existing-restore baseline, initial-baseline rollback, idempotent promotion, malformed report roots, typed atomic-write failure, persistence reload, semantic history validation, history-capacity fail-closed behavior, and rollback. |

## Verification

- `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tests -p 'test_inference_qualification*.py' -v` — 38 passed (35 qualification/store tests plus 3 CLI/transport tests).
- `PYTHONDONTWRITEBYTECODE=1 python3 -m compileall -q albert_mvp tests/test_inference_qualification*.py` — passed.
- `ruff format --check` passed for the qualification module and tests; `git diff --check` passed.
