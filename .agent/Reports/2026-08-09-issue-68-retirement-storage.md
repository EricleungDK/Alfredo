...
**Status:** Implemented locally; third-review corrections in validation
...
`retirement-export` persists the exact correlation, revision, destination, export kind, and manifest/tree digest before reconstructing retained material. A `retirement-blocked` unit reconstructs its verified Snapshot Payload. A `preservation-blocked` unit exports the exact quiesced managed path even when broken Git metadata caused preservation to block; it fingerprints and re-verifies files, modes, directories, and symlinks without following links, and excludes the live Git administration pointer. The destination must be outside both the retained source and app-private runtime. A crash after materialization but before the marker or canonical receipt is recoverable only after exact exported-content verification. Any changed export remains blocked and is never overwritten.

Retained Worktree Discard requires the exact session id as confirmation, an explicit reason, a terminal blocked unit, fresh independent Runner Quiescence and open-handle absence, the deterministic managed worktree path, exact root identity and retained-content digest, and Coding Workspace exclusion. It persists those boundaries before moving/removing the exact path. Snapshot-backed Git cleanup remains non-force; explicit no-snapshot discard may force-remove the separately fingerprinted exact Git worktree, or isolate and delete the exact managed directory when broken metadata prevents Git identity proof. It proves final path and applicable Git-registration absence, records the irreversible outcome, and releases a still-bound Preservation Budget only after deletion. A replacement directory is rejected without deleting its bytes. Pin, retry, export, and discard share the per-session effect lock, so no concurrent blocked action can strand an already completed deletion. Exact correlation replay never repeats an effect or overwrites newer canonical state; changed-boundary reuse fails closed.
...
- Retirement preservation/lifecycle/storage coverage: 81 tests plus 4 subtests passing, including every first-, second-, and third-review executable reproduction and direct typed CLI action dispatch.
...
The first required review against `aa92ab36ec92280b696e758f60e6da07362e54b5...6940130` found three Standards gaps, five Spec gaps plus one admission race, and six executable Correctness/Risk defects. Correction commit `6fb582a` addressed that set. The second review found owner-unbound receipts, a corrupting preservation retry, retry/effect concurrency, unsupported inspect controls, fabricated empty records, read-side reclamation, missing preservation-blocked export, missing danger styling, and inaccurate pin guidance. Commit `85350be` covered that set. The third review found a broken no-snapshot discard path, direct export's unnecessary Git-identity and destination gaps, receipt ownership missing from persisted authority, and non-idempotent unpin reclamation plus raw UI state. The current correction binds action receipts to Mission/session, makes unpin a deletion-free policy mutation, fingerprints direct discard before isolation, admits broken-Git export at safe destinations, and shares one typed human-readable retirement presenter.

## Remaining Gate

Commit and validate the third correction set, then repeat independent parallel Standards, Spec, and Correctness/Risk review against fixed point `aa92ab36ec92280b696e758f60e6da07362e54b5`. Resolve every supported finding before marking this report complete. No push, pull request, or GitHub tracker mutation is part of this implementation run.
