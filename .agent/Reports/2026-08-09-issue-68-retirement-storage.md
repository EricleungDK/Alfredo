...
**Status:** Implemented locally; second review corrections validated and final clean re-review pending
...
- Capacity pressure reclaims only expired, unpinned payloads whose Retirement Unit is already `retired`. Ordering is deterministic by creation time and session id. Startup and explicit unpin sweep all such expired payloads even without new admission pressure. `/storage` and `/status` remain read-only. Grace, preservation/retirement-blocked, pinned, and other protected records remain untouched.
...
`retirement-export` persists the exact correlation, revision, destination, export kind, and manifest/tree digest before reconstructing retained material. A `retirement-blocked` unit reconstructs its verified Snapshot Payload. A `preservation-blocked` unit exports the exact quiesced managed worktree after path and Worktree Identity proof; it fingerprints and re-verifies files, modes, directories, and symlinks without following links, and excludes the live Git administration pointer. A crash after materialization but before the marker or canonical receipt is recoverable only after exact exported-content verification. Any changed export remains blocked and is never overwritten.

Retained Worktree Discard requires the exact session id as confirmation, an explicit reason, a terminal blocked unit, fresh independent Runner Quiescence and open-handle absence, the deterministic managed worktree path, exact retained-content identity, and Coding Workspace exclusion. It persists intent before moving/removing the exact path with non-force Git cleanup, proves final path and applicable Git-registration absence, records the irreversible outcome, and releases a still-bound Preservation Budget only after deletion. A replacement directory is rejected without deleting its bytes. Pin, retry, export, and discard share the per-session effect lock, so no concurrent blocked action can strand an already completed deletion. Exact correlation replay never repeats an effect or overwrites newer canonical state; changed-boundary reuse fails closed.

Every persisted action receipt is validated against its action-specific shape, owning session revision, and exact discard confirmation during runtime load. The four meaningful mutations also pass through typed Workstation authority in Mission Work and the desktop bridge, use the session lifecycle revision, and append one Mission/session-linked Activity Journal entry. If the core effect commits before the Workstation receipt, exact correlation replay recovers the canonical effect and completes the missing audit side effects once.
...
- Retirement preservation/lifecycle/storage coverage: 76 tests plus 4 subtests passing, including every first- and second-review executable reproduction and direct typed CLI action dispatch.
...
The first required review against `aa92ab36ec92280b696e758f60e6da07362e54b5...6940130` found three Standards gaps, five Spec gaps plus one admission race, and six executable Correctness/Risk defects. Correction commit `6fb582a` addressed that set. The second review found owner-unbound receipts, a corrupting preservation retry, retry/effect concurrency, unsupported inspect controls, fabricated empty records, read-side reclamation, missing preservation-blocked export, missing danger styling, and inaccurate pin guidance. The current correction set covers each with owner-aware validation, one effect lock, direct preservation retry, read-only inspection, mutation-time expiry, exact direct worktree export, visible inspection facts, nullable records, and accurate styling/guidance.
