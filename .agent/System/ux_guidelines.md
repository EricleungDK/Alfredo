...
- Show `snapshot-storage-exhausted` as prominent blocked Mission Work attention with an Inspect Snapshot Storage path. A blocked Retirement Unit inspector shows its compact Retirement Record and only actions the backend says are possible: pin/unpin, bounded retry, verified export with an explicit destination, and irreversible discard with both a reason and the exact session id. Discard uses danger styling and stays disabled until the exact confirmation matches; no action may claim success before its Workstation acknowledgement and Activity Journal receipt.
