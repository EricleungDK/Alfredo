...
Issue 60 adds `WayfinderService` as the deterministic, project-level first-contact adapter in front of controller routing. It persists one active `chart | work-through` flow in `runtime_root/wayfinder/<repository-hash>/wayfinder-state.json`, keyed to the repository rather than a Mission or controller memory. A new project or consequential change enters Chart; a fresh Wayfinder map/ticket reference enters Work-through; later turns retain that one durable flow without nesting/reinvocation. Read-only explanation, status, review, diagnosis, and inspection stay outside automatic Chart. A pending Shared Understanding Gate admits conversation, inspection, bounded research, Grilling, and throwaway prototypes through the ordinary controller/command route while retaining the typed active-flow projection. It guards all canonical Mission Draft mutation, Ad Hoc Delegation, and production launch authority before mutation; `AlbertMission` enforces the same state before direct CLI/TUI issue or repair launch. Only an explicit Mission Commander receipt or a visible persisted `wayfinder-agent` acknowledgement of destination, scope, constraints, and uncertainty opens the gate. The acknowledgement ends its turn and does not invoke another skill or action. Tauri decodes the typed projection; React displays the mode/gate route without treating it as an action receipt.
