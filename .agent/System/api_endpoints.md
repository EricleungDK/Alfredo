...
| Retirement lifecycle | `retirement-preserve`, `retirement-verify`; terminal/review/cancel/repair commands and startup reconciliation | Revision-claim and verify preservation, then apply outcome-specific grace or exact replay-safe retirement without a separate merge wait |
...
Outcome transitions invoke the same canonical lifecycle automatically. Accepted reviews and completed cancellations preserve and retire after quiescence; a known-live cancelled runner stays pending rather than becoming blocked. Terminal cancellation/failure before worktree creation preserves deterministic absence, allowing retirement or grace without materializing a workspace. Failed or rejected sessions enter passive grace; the common CLI option `--retention-grace-seconds` configures the mission policy and defaults to 72 hours. Needs-human-review remains active. Startup and later reconciliation resume `preserving`, `preserved`, `grace`, or `retiring` units idempotently. Removal gets an initial attempt, one automatic short-backoff retry, and one later reconciliation/startup retry before `retirement-blocked`; per-unit locking prevents concurrent effects. Git and managed-directory removal first moves the exact worktree to a deterministic app-local effect path, then validates and deletes only that isolated path; new bytes at the original path remain intact and block completion, and restart resumes an interrupted isolated effect. Exact-content comparison accepts only the preserved state or its safe partial-cleanup subset. A missing Git path with a lingering registration resumes the same exact non-force removal boundary. Repair first materializes the verified predecessor snapshot, retires that unit, and only then persists the separately authorized queued repair session. The persistent server accepts the same argv and preserves one-process/persistent semantics. Payload reclamation, pinning, export, and explicit discard remain outside this command surface until Issue #68.
