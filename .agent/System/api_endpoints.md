...
| Retirement preservation | `retirement-preserve`, `retirement-verify` | Revision-claim one terminal Retirement Unit, prove identity/quiescence, publish its app-local snapshot, or re-run manifest/hash/clean-room verification without removing the worktree |
...
`retirement-preserve` accepts an exact session/Mission, required session `expected_revision`, and required request correlation id. Python claims the Retirement Unit before filesystem work, rejects nonterminal sessions, non-active lifecycle phases, stale revisions, unbound reservations, ambiguous or unregistered Worktree Identity, and any supervising-runner lease or process-group result other than independently corroborated `absent`. A successful response is stored as an exact correlation receipt, so a lost-response retry replays the same snapshot instead of duplicating work. Success returns the updated session revision, verified/unbound Preservation Budget, and Retirement Snapshot record. `retirement-verify` is read-only with respect to canonical state: it re-reads the exact published manifest, checks every recorded authority/size/hash boundary, then reconstructs the preserved Git state from the app-local baseline bundle, patches, and files in a disposable clean room. Neither command performs physical retirement, payload reclamation, or discard. The persistent server accepts the same CLI argv and therefore preserves one-process/persistent semantic equivalence.
