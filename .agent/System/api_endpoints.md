...
`--snapshot-storage-retention-seconds` and `--snapshot-storage-budget-bytes` configure the common mission policy and default to 30 days and 5 GiB. Every new session admission counts retained payload bytes plus bound Preservation Budgets and reserves another 32 MiB. When needed, Python persists a reclamation intent and removes expired, unpinned, retired payloads oldest first. Protected or pinned exhaustion persists `snapshot-storage-exhausted` attention and rejects admission. `retirement-storage` is read-only and returns policy, payload/reserved/committed/available bytes, record/retained/pinned/reclaimed/expired-eligible counts, ordered expiry, ten largest retained payloads, reclamation history, and blockers. Agent Console `/status` includes the compact totals; `/storage` renders the deterministic inspection without model inference.

`retirement-inspect` returns one exact Retirement Unit revision, terminal/phase/blocker state, Worktree Identity and runner boundary, Preservation Budget, compact Retirement Record, and available actions. `retirement-pin` changes only a retained payload's pin state. `retirement-retry` durably authorizes one fresh bounded attempt from `preservation-blocked` or `retirement-blocked`. `retirement-export` requires a blocked unit with a verified retained payload and a nonexistent explicit destination; it persists intent before reconstruction and recovers a lost marker/receipt only after proving exact exported content and Git state. `retirement-discard` requires a blocked terminal unit, exact session-id confirmation, reason, expected revision, fresh Runner Quiescence, deterministic managed containment, and Coding Workspace exclusion before exact deletion. All mutations require correlation ids, replay exact receipts without repeating effects, reject changed-boundary reuse, and share one-process/persistent semantics.
