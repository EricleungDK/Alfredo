...
When a Wayfinder flow has a pending gate, every Mission Draft mutation, `ad-hoc-delegation-proposal`, approval of an Ad Hoc Delegation, `workstation-action` session launch, and direct `launch`/TUI launch-or-repair command rejects with the recoverable stable error `shared-understanding-required` before it mutates canonical state. Read-only/discovery boundaries remain available.
