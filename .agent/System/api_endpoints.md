...
Outcome transitions invoke the same canonical lifecycle automatically. Accepted reviews and completed cancellations preserve and retire immediately after quiescence. Failed or rejected sessions enter passive grace; the common CLI option `--retention-grace-seconds` configures the mission policy and defaults to 72 hours. Needs-human-review remains active. Startup and later reconciliation resume any `preserving`, `preserved`, `grace`, or `retiring` unit idempotently. Each removal effect is attempted at most three times before `retirement-blocked`; a crash after the filesystem effect but before the retired receipt finalizes from verified absence without repeating deletion. Repair first materializes the verified predecessor snapshot, retires that unit, and only then persists the separately authorized queued repair session. The persistent server accepts the same argv and preserves one-process/persistent semantics. Payload reclamation, pinning, export, and explicit discard remain outside this command surface until Issue #68.
