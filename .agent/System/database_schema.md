...
- Valid Evidence Packages persist `evidence:<mission-id>:<session-id>` on the owning session in the same runtime write. Exact replay of the same package is a no-op; trying to reuse that identity for changed evidence fails closed. Legacy or tampered evidence without the exact Mission-qualified stored identity remains reviewable in its older data path but does not produce a receipt-styled evidence chronology claim.
