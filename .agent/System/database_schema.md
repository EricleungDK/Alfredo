...
| `runtime.json` | `AlbertMission` | mission, issues, sessions, reviews, delegations, command policy, `workstation_actions`, persisted evidence correlation ids, timeline | Canonical mission runtime, Local Agent lifecycle, evidence identity, and mutation-coincident Workstation/review recovery markers |
...
| `wayfinder-state.json` | `WayfinderService` | schema version, one active flow, mode, originating prompt message id, gate status/opening receipt | Project-level canonical Wayfinder entry and Shared Understanding Gate state; no controller memory is authority |
...
- A project has at most one active Wayfinder flow. Its `chart | work-through` mode, originating exact prompt identity, and `pending | open` Shared Understanding Gate survive restart in `wayfinder-state.json`; only an explicit Mission Commander confirmation or visible structured agent acknowledgement can open that gate.
- While an active Wayfinder gate is pending, canonical Mission Draft creation/confirmation, Ad Hoc Delegation proposal/approval, and production Workstation session launch reject before mutation. Conversation, read-only inspection, bounded research, Grilling, and throwaway prototypes remain outside those guarded authorities.
...
- 2026-08-03 added `wayfinder-state.json` schema version 1. Missing state means no active Wayfinder flow; malformed state fails as a structured persistence error rather than falling back to controller memory.
