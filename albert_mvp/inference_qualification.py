...
_BASELINE_BOOTSTRAP_BLOCKERS = frozenset(
    {
        "baseline-required",
        "rollback-not-tested",
    }
...
def _validate_profile_identity(profile: LocalInferenceProfile) -> None:
    _utf8_size(profile.profile_id, "qualification profile id must be valid UTF-8")
    _utf8_size(profile.model, "qualification model name must be valid UTF-8")
    _utf8_size(profile.model_digest, "qualification model digest must be valid UTF-8")
...
class GovernedFixture:
    ...
    def __post_init__(self) -> None:
        ...
        _utf8_size(self.fixture_id, "governed fixture id must be valid UTF-8")
        ...
        if (
            _utf8_size(self.prompt, "governed fixture prompt must be valid UTF-8")
            > _MAX_FIXTURE_PROMPT_BYTES
        ):
...
class RuntimePin:
    ...
    def __post_init__(self) -> None:
        ...
        _utf8_size(self.runtime_id, "promotion runtime id must be valid UTF-8")
        _utf8_size(
            self.runtime_version,
            "promotion runtime version must be valid UTF-8",
        )
        for name, value in (
...
def _validated_rollback_claim(
    ...
    *,
    ...
    _utf8_size(previous_report_id, "rollback test report id must be valid UTF-8")
    return {
...
def _is_control_error(fixture_kind: str, error_code: str) -> bool:
    return fixture_kind == "cancellation" or error_code in _CONTROL_ERROR_CODES


def _reliability_scope(observations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Exclude cancellation-control fixtures from the work reliability gate."""

    return [item for item in observations if item["fixture_kind"] != "cancellation"]
...
class ContextSource:
    ...
    def __post_init__(self) -> None:
        ...
        _utf8_size(self.source_id, "context source id must be valid UTF-8")
        if not isinstance(self.content, str):
            raise ValueError("context source content must be text")
        if (
            _utf8_size(self.content, "context source content must be valid UTF-8")
            > _MAX_CONTEXT_SOURCE_BYTES
        ):
...
class _RequiredContextBudgetError(ValueError):
    """A required source cannot fit the bounded prompt headroom."""
        ...
        *,
        ...
        for source in required:
            ...
            if next_count > budget_tokens:
                raise _RequiredContextBudgetError(
...
class PromptPrefixReuseTracker:
    ...
    def reset(self) -> None:
        """Start a new report-local prefix evidence sequence."""

        self._last_prefix_digest = None
    ...
    def observe(self, prefix: str) -> PrefixReuseObservation:
        ...
        try:
            ...
            raise ValueError("prompt prefix must be valid UTF-8") from exc
...
class QualificationReport:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "QualificationReport":
        ...
        _utf8_size(report_id, "qualification report id is invalid")
        ...
        for fixture_id in raw_fixture_ids:
            _utf8_size(fixture_id, "qualification fixture id is invalid")
        if any(
        ...
        fixture_digests_complete = data.get("fixture_digests_complete")
        if (
        ...
        _utf8_size(baseline_report_id, "qualification baseline report id is invalid")
        raw_blockers = list(raw_blockers)
        if (
            not fixture_digests_complete
            and "fixture-digests-incomplete" not in raw_blockers
        ):
            raise ValueError("qualification fixture digest evidence is incomplete")
        if not isinstance(data.get("promotion_ready"), bool):
            raise ValueError("qualification report scalar fields are invalid")
        if data.get("promotion_ready") != (not raw_blockers):
            raise ValueError("qualification report scalar fields are invalid")
        try:
            rollback_evidence = (
            ...
            allowed_blockers = [
                ...
                *(
                    ...
                    for blocker in (
                        ...
                        "latency-regression",
                        "latency-evidence-incomplete",
                        "baseline-incomparable",
                        "baseline-not-rollback-safe",
                    )
...
class QualificationReportStore:
    ...
    def _cleanup_temporary_files(self, directory: Path, pattern: str) -> None:
        if not directory.exists():
            return
        try:
            temporary_files = heapq.nsmallest(
                self._MAX_TEMPORARY_FILES,
                directory.glob(pattern),
                key=lambda path: path.name,
            )
        except OSError as exc:
            raise PromotionError(
                "qualification temporary storage is unavailable"
            ) from exc
        for temporary in temporary_files:
            try:
                ...
                raise PromotionError(
                    "qualification temporary storage could not be cleaned"
                ) from exc
        ...
        with self._state_lock():
            ...
            if report.baseline_report_id:
                ...
                if tuple(expected_blockers) != report.promotion_blockers:
                    raise PromotionError(
                        "qualification report baseline comparison is invalid"
                    )
            ...
            self._cleanup_temporary_files(self.reports_root, ".*.json.*.tmp")
    ...
    def _validate_rollback_target(
        ...
        *,
        ...
        if (
            ...
            raise PromotionError(
                "qualification report rollback target does not match baseline"
            )
        if (
            baseline_report is not None
            and previous_report_id == baseline_report.report_id
        ...
        if set(previous_report.promotion_blockers) - _BASELINE_BOOTSTRAP_BLOCKERS:
            raise PromotionError(
                "qualification report rollback target is not rollback-safe"
            )
    ...
    def _validate_history_semantics(self, state: Mapping[str, Any]) -> None:
        ...
        for item in state.get("history", []):
            ...
            if item["action"] == "promote":
                if not report.promotion_ready:
                    raise PromotionError("promotion history report is not ready")
                target_report_id = report.rollback_evidence.get("previous_report_id")
                if not isinstance(target_report_id, str) or not target_report_id:
                    raise PromotionError("promotion history rollback target is invalid")
                if active_report_id is None:
                    if (
                        not report.baseline_report_id
                        or target_report_id != report.baseline_report_id
                    ):
                        raise PromotionError(
                            "promotion history initial rollback target is invalid"
                        )
                elif target_report_id != active_report_id:
                    raise PromotionError(
                        "promotion history rollback target is discontinuous"
                    )
                self.load_report(target_report_id)
                previous_report_id = target_report_id
                ...
                if active_report_id != report.report_id or previous_report_id is None:
                    raise PromotionError("promotion history rollback is discontinuous")
                restored_report = self.load_report(previous_report_id)
                active_report_id = restored_report.report_id
                if restored_report.promotion_ready:
                    restored_target_id = restored_report.rollback_evidence.get(
                        "previous_report_id"
                    )
                    if (
                        not isinstance(restored_target_id, str)
                        or not restored_target_id
                    ):
                        raise PromotionError(
                            "promotion history restored rollback target is invalid"
                        )
                    self.load_report(restored_target_id)
                    previous_report_id = restored_target_id
                else:
        ...
        state_active_report_id = (
            state["active"]["report_id"] if state.get("active") is not None else None
        )
        state_previous_report_id = (
            state["previous"]["report_id"]
            if state.get("previous") is not None
            else None
        )
        if (
            active_report_id != state_active_report_id
            or previous_report_id != state_previous_report_id
        ):
            raise PromotionError("promotion state does not match promotion history")
        ...
        with self._state_lock():
            ...
            if active is not None and active.get("profile_id") == profile_id:
                if (
                    ...
                    raise PromotionError("qualification report is already promoted")
    ...
    def rollback(
        ...
        *,
        ...
        with self._state_lock():
            ...
            restored_report = self.load_report(previous["report_id"])
            next_previous: dict[str, Any] | None = None
            if restored_report.promotion_ready:
                restored_target_id = restored_report.rollback_evidence.get(
                    "previous_report_id"
                )
                if not isinstance(restored_target_id, str) or not restored_target_id:
                    raise PromotionError("restored promotion has no rollback target")
                restored_target = self.load_report(restored_target_id)
                next_previous = {
                    "profile_id": restored_target.profile["profile_id"],
                    "report_id": restored_target.report_id,
                    "profile": dict(restored_target.profile),
                    "runtime_pin": dict(restored_target.runtime_pin),
                    "promoted_at": restored_target.created_at,
                }
            if len(state.get("history", [])) >= _MAX_PROMOTION_HISTORY:
                raise PromotionError("promotion history capacity exhausted")
    ...
    def _report_path(self, report_id: str) -> Path:
        ...
        try:
            report_digest = hashlib.sha256(report_id.encode("utf-8")).hexdigest()
        except UnicodeError as exc:
            raise PromotionError("qualification report id is invalid") from exc
        return self.reports_root / f"{report_digest}.json"
    ...
    def _report_id_from_path(self, path: Path) -> str:
        ...
        try:
            expected_path = self._report_path(report_id)
        except PromotionError:
            raise
        if not isinstance(report_id, str) or expected_path != path:
            raise PromotionError(
                "qualification report identity does not match its path"
            )
        return report_id
    ...
    def _validated_state(cls, payload: Any) -> dict[str, Any]:
        ...
        if set(payload) - allowed_state_fields or not required_state_fields.issubset(
            payload
        ):
        ...
        if (
            ...
            or any(
                ...
                or item.get("action") not in {"promote", "rollback"}
        ...
        for item in history:
            try:
                _utf8_size(item["profile_id"], "promotion state profile id is invalid")
                _utf8_size(item["report_id"], "promotion state report id is invalid")
                _utf8_size(
                    item["correlation_id"],
                    "promotion state correlation id is invalid",
                )
            except ValueError as exc:
                raise PromotionError("promotion state identity is invalid") from exc
        ...
        if (
            ...
            or (history and payload["last_action"] != history[-1]["action"])
        ):
            raise PromotionError("promotion state is invalid")
        try:
    ...
    def _validated_promotion_record(value: Any) -> None:
        ...
        try:
            for field_name in ("profile_id", "report_id", "promoted_at"):
                _utf8_size(value[field_name], "promotion state identity is invalid")
        except ValueError as exc:
            raise PromotionError("promotion state identity is invalid") from exc
        ...
        try:
            _utf8_size(
                ...
                "promotion state profile id is invalid",
            )
            _utf8_size(
                canonical_profile["model"],
                "promotion state model name is invalid",
            )
            _utf8_size(
                canonical_profile["model_digest"],
                "promotion state model digest is invalid",
            )
    ...
    def _mutation_digest(
        ...
        *,
        ...
        if not isinstance(profile_id, str) or not profile_id.strip():
            raise PromotionError("qualification mutation profile id is required")
        if not isinstance(report_id, str):
            raise PromotionError("qualification mutation report id is invalid")
        if (
        ...
        try:
            _utf8_size(profile_id, "qualification mutation profile id is invalid")
            _utf8_size(report_id, "qualification mutation report id is invalid")
            _utf8_size(
                correlation_id,
                "qualification mutation correlation id is invalid",
            )
        except ValueError as exc:
            raise PromotionError("qualification mutation identity is invalid") from exc
        if (
    ...
    def _write_state(self, state: dict[str, Any]) -> None:
        ...
        self._cleanup_temporary_files(self.root, ".promotion-state.json.*.tmp")
        self._cleanup_temporary_files(self.reports_root, ".*.json.*.tmp")
        self._atomic_write_text(
            self.state_path,
...
class InferenceQualificationService:
    ...
    def qualify(
        ...
        *,
        ...
        if report_id is not None:
            if not isinstance(report_id, str) or not report_id.strip():
                raise ValueError("qualification report id must be non-empty")
            _utf8_size(report_id, "qualification report id must be valid UTF-8")
        if baseline_report is not None and not isinstance(
            baseline_report, QualificationReport
        ):
            raise TypeError(
                "qualification baseline_report must be a QualificationReport"
            )
                ...
                if cancellation_requested:
                    ...
                    try:
                        if context_sources:
                            ...
                            prompt_prefix = (
                                f"{selection.prompt_prefix}\n\n{fixture.prompt}"
                            )
                            prefix = self.prefix_tracker.observe(
                                selection.prompt_prefix
                            )
                        else:
                            prompt_prefix = fixture.prompt
                            prefix = self.prefix_tracker.observe(fixture.prompt)
                    except _RequiredContextBudgetError:
                        result = control_failure_result(
                            "context-over-budget",
                            outcome="rejected",
                        ...
                        context = {
                            ...
                            "source_ids": (
                                selection.source_ids if context_sources else ()
                            ),
                        ...
                        if cancellation_requested:
                            ...
                            or fixture.required_context_tokens
                            > available_context_tokens
                        ):
                            result = control_failure_result(
                                "context-over-budget",
                            ...
                            try:
                                raw_result = runner(
                                    fixture, profile, context, repetition
                                )
                            except (
                                Exception
                            ):  # a failed cohort is evidence, not authority
                                result = control_failure_result("runner-failed")
                            else:
                                try:
        ...
        *,
        ...
        for observation in observations:
            ...
            _utf8_size(
                ...
                "qualification observation profile id is invalid",
            )
            _utf8_size(
                observation["model_digest"],
                "qualification observation model digest is invalid",
            )
            ...
            if observation_identity_complete:
                ...
                if prefix_digest:
                    if previous_prefix_digest is not None:
                        ...
                        observation["prefix_reused"]
                        or observation["prefix_invalidated"]
                    ):
                        raise ValueError(
                            "qualification observation prefix semantics are invalid"
                        )
    ...
    def _metrics(cls, observations: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        reliability_observations = _reliability_scope(observations)
        quality = sum(float(item["quality_score"]) for item in observations)
        reliable = sum(1 for item in reliability_observations if item["reliable"])
        return {
            "sample_count": count,
            ...
            "reliability_rate": (
                round(reliable / len(reliability_observations), 4)
                if reliability_observations
                else 0.0
            ),
    ...
    def _promotion_blockers(
        *,
        ...
        reliability_observations = _reliability_scope(observations)
        if not reliability_observations or any(
            not item["reliable"] for item in reliability_observations
        ):
        ...
        if baseline_report is None:
            ...
            if set(baseline_report.promotion_blockers) - _BASELINE_BOOTSTRAP_BLOCKERS:
                blockers.append("baseline-not-rollback-safe")
            if (
            ...
            if any(
                candidate_latency_summary.get(percentile) is None
                or baseline_latency_summary.get(percentile) is None
                for percentile in latency_percentiles
            ):
                blockers.append("latency-evidence-incomplete")
            elif any(
                candidate_latency_summary[percentile]
